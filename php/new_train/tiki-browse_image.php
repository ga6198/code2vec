<?php
// $Header: /cvsroot/tikiwiki/tiki/tiki-browse_image.php,v 1.18.2.3 2004/03/28 09:59:50 damosoft Exp $
...
// The $scaleinfo array is sorted from small scales to large scales.
// The size of a scale is simply the amount of pixels it contains.
while (list ($key, $val) = each ($scaleinfo)) {
  if ($val['xsize']*$val['ysize'] < $sxsize*$sysize) {
    // We've found (another) scale that is smaller than the current
    // image size.
    $prevx = $val['xsize'];
    $prevy = $val['ysize'];
    $prevt = 's';
  ...
  if ($val['xsize']*$val['ysize'] > $sxsize*$sysize) {
    // We've found a scale that is larger than the image size.  We
    // break on the first scale we find because we want the smallest
    // scale that's larger than the image size.
    $nextx = $val['xsize'];
    $nexty = $val['ysize'];
    $nextt = 's';
    break;
